<x-layout title="Ubah Data Gudang" >
    @section('content')
    <div class="p-4 my-3 mx-auto max-w-xl bg-white font-[sans-serif]">
        <h1 class="text-3xl text-gray-800 font-extrabold text-center">Pengisian Data Gudang</h1>
        <form method="POST" action="/update/gudang/{{ $id }}" class="mt-8 space-y-4">
            @method('PUT')
            @csrf
            @foreach ($Gudang as $tempat)
                <input type='text' value="{{ $tempat['nm_tempat'] }}" name="nm_tempat" placeholder='gd-Cakung'
                ...
                @error('nm_tempat')
                    <p class="text-xs text-red-500 flex items-center mt-2">
                        ...
                        {{ $errors->first('nm_tempat') }}
                    </p>
                @enderror
                <textarea name="lokasi" placeholder='Jln.Mahabarata' rows="6"
                class="w-full rounded-md px-4 text-gray-800 bg-gray-100 focus:bg-transparent text-sm pt-3 outline-blue-500">{{ $tempat['lokasi'] }}</textarea>
                @error('lokasi')
                    <p class="text-xs text-red-500 flex items-center mt-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" fill="currentColor" class="mr-2"
                            viewBox="0 0 512 512">
                ...
                @enderror
                <input type='text' value="{{ $tempat['nm_supervisor'] }}" name="nm_supervisor" placeholder='Joni SM'
                class="w-full rounded-md py-3 px-4 text-gray-800 bg-gray-100 focus:bg-transparent text-sm outline-blue-500" />
                @error('nm_supervisor')
                    <p class="text-xs text-red-500 flex items-center mt-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" fill="currentColor" class="mr-2"
                        ...
                        {{ $errors->first('nm_supervisor') }}
                    </p>
                @enderror
                <input value="{{ $tempat['kapasitas'] }}" type='text' name="kapasitas" placeholder='12.000'
                class="w-full rounded-md py-3 px-4 text-gray-800 bg-gray-100 focus:bg-transparent text-sm outline-blue-500" />
                @error('kapasitas')
                <p class="text-xs text-red-500 flex items-center mt-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" fill="currentColor" class="mr-2"
