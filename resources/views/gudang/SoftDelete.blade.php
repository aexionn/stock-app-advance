    ...
    @section('content')
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                ...
                <table class="min-w-full bg-white">
                    <thead class="bg-gray-800 whitespace-nowrap">
                        <tr>
                            ...
                            <th class="p-4 text-left text-sm font-medium text-white">Berdiri Sejak</th>
                            <th class="p-4 text-left text-sm font-medium text-white">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="whitespace-nowrap">
                        @foreach ($soft as $tempat)
                            <tr class="even:bg-blue-50">
                                <td class="p-4 text-sm text-black">{{ $tempat['nm_tempat'] }}</td>
                                <td class="p-4 text-sm text-black">{{ $tempat['lokasi'] }}</td>
                                <td class="p-4 text-sm text-black">{{ $tempat['nm_supervisor'] }}</td>
                                <td class="p-4 text-sm text-black">{{ $tempat['kapasitas'] }}</td>
                                <td class="p-4 text-sm text-black">{{ $tempat['created_at'] }}</td>
                                <td class="flex items-center">
                                    <a onclick="return confirm('Apa anda yakin ingin mengembalikan Data ini')" class="inline-block w-5 fill-green-500 hover:fill-green-700" href="/restore/gudang/{{ $tempat['id_gudang'] }}" title="Ubah">
                                    ...
                                    <form action="/delPermanent/gudang/{{ $tempat['id_gudang'] }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" onclick="return confirm('Apakah anda yakin ingin menghapus {{ $tempat['nm_gudang'] }}')" class="m-4" title="Hapus">
            ...
            <div class="font-[sans-serif] overflow-x-auto flex flex-row">
                <a href="/restore/gudang" type="button"
                ...
                <form action="/delPermanent/gudang" method="post">
                    @csrf
                    @method('delete')
                    <button type="submit"
