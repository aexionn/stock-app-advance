<x-layout title="Simpan Data Penyimpanan" >
    @section('content')
    <div class="p-4 my-3 mx-auto max-w-xl bg-white font-[sans-serif]">
        <h1 class="text-3xl text-gray-800 font-extrabold text-center">Pengisian Data Penyimpanan</h1>
        <form method="post" action="/insert/penyimpanan/{{ $id }}" class="mt-8 space-y-4">
            @csrf
            <input type='text' name="kode" value="{{ old('kode') }}" placeholder='TES-A3'
            ...
            @enderror
            <select class="w-full rounded-md py-3 px-4 text-gray-800 bg-gray-100 focus:bg-transparent text-sm outline-blue-500" name="id_gudang" id="">
                @foreach ($gudang as $items)
                    <option value="{{ $items['id_gudang'] }}">{{ $items['id_gudang'] }} - {{ $items['nm_tempat'] }}</option>
                @endforeach
            </select>
            @error('id_gudang')
                <p class="text-xs text-red-500 flex items-center mt-2">
                    ...
                    {{ $errors->first('id_gudang') }}
                </p>
            @enderror
            <input type='number' value="{{ old('jumlah_barang') }}" name="jumlah_barang" placeholder='12.000'
                class="w-full rounded-md py-3 px-4 text-gray-800 bg-gray-100 focus:bg-transparent text-sm outline-blue-500" />
            @error('jumlah_barang')
                <p class="text-xs text-red-500 flex items-center mt-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" fill="currentColor" class="mr-2"
