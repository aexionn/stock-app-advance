    ...
    <div class="p-4 my-3 mx-auto max-w-xl bg-white font-[sans-serif]">
        ...
        <form method="POST" action="/update/barang/{{ $id }}" class="mt-8 space-y-4">
            ...
            @foreach ($Barang as $items)
                <input type='text' value="{{ $items['nm_barang'] }}" name="nm_barang" placeholder='Apel'
                ...
                class="w-full rounded-md px-4 text-gray-800 bg-gray-100 focus:bg-transparent text-sm pt-3 outline-blue-500">{{ $items['deskripsi'] }}</textarea>
                @error('deskripsi')
                    <p class="text-xs text-red-500 flex items-center mt-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" fill="currentColor" class="mr-2"
                            viewBox="0 0 512 512">
                ...
                @enderror
                <input value="{{ $items['harga'] }}" type='text' name="harga" placeholder='12.000'
                class="w-full rounded-md py-3 px-4 text-gray-800 bg-gray-100 focus:bg-transparent text-sm outline-blue-500" />
                @error('harga')
                    <p class="text-xs text-red-500 flex items-center mt-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" fill="currentColor" class="mr-2"
