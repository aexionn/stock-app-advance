<x-layout :title="$title">
    <main>
        @section('content')
        <div x-data="{isOpen:false}" class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                ...
                <table class="min-w-full bg-white">
                    <thead class="bg-gray-800 whitespace-nowrap">
                        <tr>
                            ...
                            <th class="p-4 text-left text-sm font-medium text-white">Stok Terbaru</th>
                            ...
                            <th class="p-4 text-left text-sm font-medium text-white">Jumlah Barang</th>
                            <th class="p-4 text-left text-sm font-medium text-white">Ditambahkan Pada</th>
                            <th class="p-4 text-left text-sm font-medium text-white">Diubah Pada</th>
                    ...
                    <tbody class="whitespace-nowrap">
                        ...
                        <tr class="even:bg-blue-50">
                            ...
                            {{-- @dd($items->penyimpanan) --}}
                            {{-- @dd($item) --}}
                            {{-- <td class="p-4 text-sm text-black">{{ $item['stok_awal'] ?? '' }}</td> --}}
                            <td class="p-4 text-sm text-black">{{ $item['stok_terbaru'] ?? '' }}</td>
                            <td class="p-4 text-sm text-black">{{ $item->gudang->pluck('nm_tempat')  ?? '' }}</td>
                            <td class="p-4 text-sm text-black">{{ $item->gudang->pluck('kapasitas') ?? '' }}</td> 
                            <td class="p-4 text-sm text-black">
                                @foreach ($item->gudang as $items)
                                    <p>{{ $items->penyimpanan->jumlah_barang ?? '' }}</p> 
                                @endforeach
                            </td>
                            <td class="p-4 text-sm text-black">
                                @foreach ($item->gudang as $items)
                                    <p>{{ $items->penyimpanan->ditambahkan_pada ?? '' }}</p> 
                                @endforeach
                            </td>
                            <td class="p-4 text-sm text-black">
                                @foreach ($item->gudang as $items)
                                    <p>{{ $items->penyimpanan->diubah_pada ?? '' }}</p> 
                                @endforeach
                            </td>
                            <td class="p-4 text-sm text-black">
                                <a href="/save/penyimpanan/{{ $item['id_barang'] }}" class="inline-block" title="Ubah">
            ...
            <div class="font-[sans-serif] overflow-x-auto flex flex-row">
                ...
                <a href="/softDelInfo/penyimpanan" type="button"
